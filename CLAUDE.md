...
- **System Restore:** `SystemRestoreService` orchestrates full-system restore (DB → MinIO → PKI) with SignalR live log streaming. Endpoints at `/api/admin/system-restore/*`. Frontend tab "Toàn hệ thống" in Admin → Backup/Restore. See `docs/backup_and_restore.md` Section 14.
- **WAL Backup to S3:** On-demand WAL archive + S3 upload via `POST /api/admin/data-backup/compliance/wal/backup-to-s3`. Uses `WalBackupSchedulerService.RunOnDemandArchiveAsync()`. See `docs/backup_and_restore.md` Section 15.
