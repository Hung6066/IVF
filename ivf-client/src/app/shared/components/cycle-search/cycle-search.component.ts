...
@Component({
  ...
  template: `
    <div class="cycle-search-container">
      ...
      @if (
        showDropdown &&
        (results().length > 0 || searchTerm || (showHistory && historyItems().length > 0))
      ) {
            ...
            @for (c of historyItems(); track c.id) {
              <div class="dropdown-item" (click)="selectCycle(c)">
                ...
                <div class="cycle-info">
                  <div class="fw-bold">{{ c.wifeName || c.method }}</div>
                  <div class="text-muted small">{{ c.method }} · {{ formatDate(c.startDate) }}</div>
                </div>
                <button
                  type="button"
                  class="delete-history-btn"
                  (click)="removeHistoryItem($event, c)"
            ...
            <div class="dropdown-item" (click)="selectCycle(c)">
              ...
              <div class="cycle-info">
                ...
                <div class="text-muted small">
                  {{ c.method }} · {{ c.currentPhase }} · {{ formatDate(c.startDate) }}
                </div>
              </div>
            </div>
          }
          @if (results().length === 0 && searchTerm) {
            <div class="dropdown-item empty">Không tìm thấy chu kỳ "{{ searchTerm }}"</div>
          }
        </div>
      }
...
export class CycleSearchComponent implements OnInit, OnChanges, ControlValueAccessor {
  ...
  ngOnInit() {
    this.searchSubject
      ...
      .subscribe({
        next: (data: any) => {
          // Assuming data is array or wrapper
          this.isLoading.set(false);
  ...
  onSearch(event: Event) {
    ...
    if (!term) {
      if (!this.patientId) {
        // Only show history if no patientId filter
        this.showHistory = true;
        this.historyItems.set(this.recentItemsService.getItems('cycle'));
      } else {
        this.showHistory = false;
