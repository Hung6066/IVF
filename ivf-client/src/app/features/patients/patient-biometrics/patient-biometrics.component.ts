...
import {
  FingerprintHubService,
  CaptureResult,
  CaptureStatus,
  EnrollmentProgress,
  VerificationResult,
} from '../../../core/services/fingerprint/fingerprint-hub.service';
    ...
    this.route.params.subscribe((params) => {
      ...
      if (id) {
        ...
        if (this.useSignalR()) {
          ...
          this.fingerprintHub.connect().then(() => {
      ...
      this.fingerprintHub.verificationResult$.subscribe((result) => {
        if (result.success) {
          this.notificationService.success(
            'Xác thực',
            `✅ KHỚP mẫu vân tay: ${this.getFingerTypeName(result.fingerType || '')}`,
          );
        } else {
    ...
    try {
      ...
      if (connected) {
        ...
        this.notificationService.info(
          'SignalR',
          'Đã kết nối - Mở ứng dụng DigitalPersona trên máy tính',
        );
      } else {
  ...
  async captureFingerprint(): Promise<void> {
    ...
    try {
      ...
      if (result) {
        ...
        this.notificationService.success(
          'Thành công',
          `Đã chụp vân tay (Chất lượng: ${result.quality}%)`,
        );
      } else {
  ...
  async verifyFingerprint(): Promise<void> {
    if (!this.useSignalR() || !this.signalRConnected()) {
      this.notificationService.warning(
        'Cảnh báo',
        'Cần kết nối với ứng dụng máy tính qua SignalR để xác thực nhanh',
      );
      return;
    }
