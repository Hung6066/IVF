...
import { FormsTabComponent } from './tabs/forms-tab.component';
...
@Component({
  ...
  imports: [
    CommonModule,
    RouterModule,
    IndicationTabComponent,
    StimulationTabComponent,
    CultureTabComponent,
    TransferTabComponent,
    LutealTabComponent,
    PregnancyTabComponent,
    BirthTabComponent,
...
export class CycleDetailComponent implements OnInit {
  ...
  loadData(id: string) {
    ...
    this.ultrasoundService
      .getUltrasoundsByCycle(id)
      .subscribe((u: Ultrasound[]) => this.ultrasounds.set(u));
  ...
  getOutcomeName(outcome?: string): string {
    const names: Record<string, string> = {
      ...
      Pregnant: 'Có thai',
      NotPregnant: 'Không thai',
      Cancelled: 'Huỷ',
      FrozenAll: 'Trữ phôi toàn bộ',
    };
  ...
  advancePhase(): void {
    ...
    if (currentIndex < this.phases.length - 1) {
      ...
      this.cycleService
        .advanceCyclePhase(c.id, nextPhase)
        .subscribe((updatedCycle: TreatmentCycle) => {
