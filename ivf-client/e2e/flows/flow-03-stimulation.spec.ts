...
import {
  ...
  apiGet, apiPost, extractItems, extractItem, getCycleIdFromApi,
} from '../helpers';
...
test.describe('Luồng 3 — API: Stimulation', () => {
  test('3.A1 — GET /stimulation/cycle/{id} tracker data', async ({ page }) => {
    ...
    } catch { /* cycle may not be in stimulation phase */ }
  });
...
test.describe('Luồng 3 — UI: Siêu âm & KTBT', () => {
  ...
  test('3.2 — Siêu âm: chuyển tab hàng đợi / lịch sử', async ({ page }) => {
    ...
    const tabs = page.locator('button, [role="tab"]').filter({ hasText: /hàng đợi|kết quả|lịch sử/i });
