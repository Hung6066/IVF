...
import {
  ...
  apiGet, apiPost, extractItems, extractItem, uniqueName, getPatientFromApi,
} from '../helpers';
  ...
  test('17.A3 — POST /prescriptions tạo đơn thuốc', async ({ page }) => {
    ...
    try {
      const res = await apiPost(page, '/prescriptions', {
        ...
        items: [
          { drugName: 'Progesterone', quantity: 30, unit: 'Viên', dosage: '1 viên/ngày', instructions: 'Sau ăn' },
        ],
  ...
  test('17.A4 — POST /prescriptions → GET by ID', async ({ page }) => {
    ...
    try {
      const createRes = await apiPost(page, '/prescriptions', {
        ...
        prescriptionDate: new Date().toISOString().split('T')[0],
        items: [
          { drugName: 'Estradiol', quantity: 14, unit: 'Viên', dosage: '1 viên/ngày', instructions: 'Sáng' },
        ],
        waiveConsultationFee: false,
      });
...
test.describe('Luồng 17: Nhà thuốc (Pharmacy)', () => {
  ...
  test('17.3 — Chuyển tab nhà thuốc', async ({ page }) => {
    ...
    const tabs = page.locator('[role="tab"], .tab, button').filter({ hasText: /Chờ|Đã phát|Tồn kho|Đơn thuốc/i });
    const count = await tabs.count();
    if (count > 0) {
