...
import {
  ...
  apiGet, apiPost, extractItems, extractItem, getCycleFromApi, getPatientFromApi,
} from '../helpers';
...
test.describe('Luồng 7 — API: Cycle Detail & Phase', () => {
  ...
  test('7.A4 — GET /stimulation/cycle/{id} stimulation data', async ({ page }) => {
    ...
    } catch { /* may not have stimulation data */ }
  });
...
test.describe('Luồng 7 — UI: Nuôi cấy & chuyển phôi', () => {
  ...
  test('7.2 — Tab Chỉ định (indication)', async ({ page }) => {
    ...
    if (!cycleId) { test.skip(); return; }
    await navigateTo(page, `/cycles/${cycleId}`);
    const indicationTab = page.locator('button.tab-btn').filter({ hasText: /Chỉ định|Indication/i }).first();
  ...
  test('7.3 — Tab Nuôi cấy (culture)', async ({ page }) => {
    ...
    if (!cycleId) { test.skip(); return; }
    await navigateTo(page, `/cycles/${cycleId}`);
    const cultureTab = page.locator('button.tab-btn').filter({ hasText: /Nuôi cấy|Culture/i }).first();
  ...
  test('7.4 — Tab Chuyển phôi (transfer)', async ({ page }) => {
    ...
    if (!cycleId) { test.skip(); return; }
    await navigateTo(page, `/cycles/${cycleId}`);
    const transferTab = page.locator('button.tab-btn').filter({ hasText: /Chuyển phôi|Transfer/i }).first();
