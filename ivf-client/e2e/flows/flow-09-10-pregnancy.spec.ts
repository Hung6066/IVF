...
test.describe('Luồng 9/10 — API: Pregnancy', () => {
  test('9.A1 — GET /pregnancy/cycle/{id} dữ liệu thai kỳ', async ({ page }) => {
    ...
    try {
      ...
      /* cycle may not have pregnancy data */
    }
  });
...
test.describe('Luồng 9/10 — UI: Thai kỳ', () => {
  test('9.1 — Trang beta-hCG khi có cycleId', async ({ page }) => {
    ...
    await expect(
      page.locator('app-pregnancy-beta-hcg, [class*="beta"], [class*="pregnancy"]').first(),
    ).toBeVisible();
  });
  ...
  test('9.2 — Beta-hCG: form nhập kết quả xét nghiệm', async ({ page }) => {
    ...
    await expect(
      page
        .locator('h1, button')
        .filter({ hasText: /Beta|Ghi nhận/i })
        .first(),
  ...
  test('10.1 — Trang prenatal khi có cycleId', async ({ page }) => {
    ...
    await expectPageLoaded(page);
    await expect(
      page.locator('app-pregnancy-prenatal, [class*="prenatal"], [class*="pregnancy"]').first(),
    ).toBeVisible();
  });
